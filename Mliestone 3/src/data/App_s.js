...
const MapComponent = () => {
  const [parameter, setParameter] = useState('RentIndex'); // Example state variable for the parameter
  ...
  useEffect(() => {
    ...
    // Fetch the CSV data
      cities.forEach(item => {
        ...
        var string = item.City + ": " + item[parameter];
        var popup = L.popup().setContent(string);
        // Create a marker or circle based on the data
        var tmp = L.circle(L.latLng(lat, lon), {
          color: 'red',
          opacity: 0,
          fillColor: '#f03',
          fillOpacity: 0.7,
          radius: 1500*parseFloat(item[parameter]) // Adjust the radius based on your data
  ...
  return (
    ...
    <h2>Your Map Title</h2> {/* Add your desired title here */}
    ...
    >
    <option value="CostofLivingIndex">Cost of Living Index</option>
    <option value="RentIndex">Rent Index</option>
    <option value="CostofLivingPlusRentIndex">Cost of Living Plus Rent Index</option>
    <option value="GroceriesIndex">Groceries Index</option>
    <option value="RestaurantPriceIndex">Restaurant Price Index</option>
    </select>
